DECLARE 
    ...
    @CodProducto NVARCHAR(50) = '17RBIAM500', -- Filtrar por producto
    @NumeroLote NVARCHAR(50) = 'F-20129';        -- Nuevo filtro por lote
...
WITH Movimientos AS (
    ...
    WHERE DC.Fecha_Compra <= @FechaCorte
      ...
      AND ISNULL(DC.Numero_Lote, 'SINLOTE') = @NumeroLote
    ...
    WHERE DF.Fecha_Factura <= @FechaCorte
      ...
      AND ISNULL(DF.CodTarea, 'SINLOTE') = @NumeroLote
)
